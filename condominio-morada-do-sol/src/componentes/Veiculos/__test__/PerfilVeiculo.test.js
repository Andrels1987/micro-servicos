  ...
  test('deve renderizar loading se proprietario não existir', () => {
   ...
   test('Exibe uma imagem padrão caso veiculo.foto seja null ou undefined', () => {
    ...
    test('chamar useGetVeiculoPeloIdQuery com os parametros corretos ', async () => {
       ...
       jest.spyOn(veiculoApi, "useGetVeiculoPeloIdQuery").mockReturnValue({ data: mockVeiculo, isLoading: false, isError: false });
       await waitFor(() => {
           expect(veiculoApi.useGetVeiculoPeloIdQuery).toHaveBeenCalledTimes(1);
           expect(veiculoApi.useGetVeiculoPeloIdQuery).toHaveBeenCalledWith({id: '1'});
       })
